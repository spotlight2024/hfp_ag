...
#define WS_GENERATOR_TICK_US        (60000)
...
static SemaphoreHandle_t ws_send_data_Semaphore = NULL;
static TaskHandle_t s_bt_app_send_data_task_handler = NULL;
static TaskHandle_t bt_ws_send_data_task_handler = NULL;
...
static uint32_t bt_app_hf_outgoing_cb(uint8_t *p_buf, uint32_t sz)
{
    ...
    if (item_size >= sz) {
        data = xRingbufferReceiveUpTo(s_m_rb, &get_size1, 0, sz);
        if (data != NULL)
        {
            memcpy(p_buf, data, get_size1);
            vRingbufferReturnItem(s_m_rb, data);
        }
        if (get_size1 < sz) {
            item_size = sz - get_size1;
            data = xRingbufferReceiveUpTo(s_m_rb, &get_size2, 0, item_size);
            if (data != NULL)
            {
                memcpy(p_buf+get_size1, data, get_size2);
                vRingbufferReturnItem(s_m_rb, data);
            }
        }
        return sz;
...
static void bt_app_hf_incoming_cb(const uint8_t *buf, uint32_t sz)
{
    ...
    if ( IsDeviceListen() )
    {
        if (m_ws_rb ) {
            ...
            if (!done) {
                ESP_LOGE(BT_HF_TAG, "bt incoming ringbuf send fail");
            }
        }
    }
    /*
...
static void bt_ws_send_data_task(void *arg)
{
    size_t item_size = 0;
    size_t get_size1 = 0;
    size_t get_size2 = 0;
...
//    size_t interval_size = get_appsr_feedbuff_size();
    ...
    for (;;) {
        //if (xSemaphoreTake(ws_send_data_Semaphore, (TickType_t)portMAX_DELAY)) {
            ...
            while (item_size >= interval_size) {
                buf = osi_malloc(interval_size);
                if (!buf) {
                    ESP_LOGE(BT_HF_TAG, "%s, no mem", __FUNCTION__);
                    continue;
                }
                
                p_buf = xRingbufferReceiveUpTo(m_ws_rb, &get_size1, 0, interval_size);
                if (p_buf != NULL) {

                    memcpy(buf, p_buf, get_size1);
                    //websocket_app_send(p_buf, get_size1);
                    vRingbufferReturnItem(m_ws_rb, p_buf);
                }

                
                if (get_size1 < interval_size) {
                    //interval_size -= get_size1;
                    p_buf = xRingbufferReceiveUpTo(m_ws_rb, &get_size2, 0, (interval_size - get_size1));
                    if ( p_buf != NULL) {
                        memcpy(buf + get_size1, p_buf, get_size2);
                        //websocket_app_send(buf, get_size2 + get_size1);
                        vRingbufferReturnItem(m_ws_rb, p_buf);
                    }
                }
...
static void bt_app_send_data_task(void *arg)
{
    ...
    for (;;) {
        if (xSemaphoreTake(s_send_data_Semaphore, (TickType_t)portMAX_DELAY)) {
            ...
            //ESP_LOGE("GGGCCC", "s_us_duration:%lld", s_us_duration);

            //if (s_us_duration > (PCM_GENERATOR_TICK_US * 5)) {
            //    s_us_duration = 0;
            //}
...
void bt_app_send_data(void)
{
    ...
    //xTaskCreatePinnedToCore(bt_app_send_data_task, "BtAppSendDataTask", 2048, NULL, configMAX_PRIORITIES - 2, &s_bt_app_send_data_task_handler,1);
    s_m_rb = xRingbufferCreate(10*1024, RINGBUF_TYPE_BYTEBUF);    
...
void bt_app_send_data_shut_down(void)
{
    ...
    if (bt_ws_send_data_task_handler) {
        vTaskDelete(bt_ws_send_data_task_handler);
        bt_ws_send_data_task_handler = NULL;
    }
    if(ws_periodic_timer) {
        ESP_ERROR_CHECK(esp_timer_stop(ws_periodic_timer));
        ESP_ERROR_CHECK(esp_timer_delete(ws_periodic_timer));
    }
    if (ws_send_data_Semaphore) {
        vSemaphoreDelete(ws_send_data_Semaphore);
        ws_send_data_Semaphore = NULL;
    }
    if (m_ws_rb) {
        vRingbufferDelete(m_ws_rb);
    }
...
void bt_app_hf_cb(esp_hf_cb_event_t event, esp_hf_cb_param_t *param)
{
    ...
    switch (event) {
        ...
        {
            if ( param->conn_stat.state == 3 || param->conn_stat.state == 2)
            {
                SetDeviceLedState(kDeviceStateBTConnected);
                //app_opus_init();
            } else 
